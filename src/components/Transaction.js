...
export default class Transaction {
  ...
  createHtmlElement = (sel) => {
    ...
    const transaction = this._html.querySelector('.transaction');
    switch(this._actionKind) {
      ...
      case ACTION_KIND.TRANSFER:
        if(this._actionDirection === ACTION_DIRECTION.IN) {
          transaction.classList.add('transaction_type_receive');
        }
        else if(this._actionDirection === ACTION_DIRECTION.OUT) {
          transaction.classList.add('transaction_type_send');
        }
      break;
    ...
    button.addEventListener('click', () => {
      if(source.classList.contains(sel.sourceVisible)) {
        button.textContent = 'Show source';
      }
      else {
        button.textContent = 'Hide source';
      }
      source.classList.toggle(sel.sourceVisible);
    });
